...
def get_dns_ip():
    ...
    a_record = cf.zones.dns_records.get(zone_id, params={"name": DOMAIN, "type": "A"})[0]
...
def update_cloud_flare(dns_ip, public_ip):
    if dns_ip and public_ip:
        ...
        if a_record["content"] != public_ip:
            print("\tUpdating A record from ({}) to ({})".format(a_record["content"], public_ip))
            old_ip = a_record["content"]
            a_record["content"] = public_ip
            
            cf.zones.dns_records.put(zone_id, a_record["id"], data=a_record)
            send_email_notification(body="Cloudflare DNS IP Updated from {} to new IP: {} !".format(old_ip, public_ip))
        else:
            print("\tCloudflare IP is already the same as the current one! Not changing anything.")
        print("\tDone")
        
def send_email_notification(body="Public IP changed", subject="IP_CHECKER", sender=SENDER_EMAIL, recipients=[RECIPIENT_EMAIL], password=SENDER_EMAIL_PASSWORD):
    msg = MIMEText(body)
    msg['Subject'] = subject
    msg['From'] = sender
